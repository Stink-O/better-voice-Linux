...
This is a native Linux port of
[BetterVoice](https://github.com/TarunTomar122/better-voice), which is a macOS
menu-bar app. The port is a tray application built on PyQt6: same idea, same
local-only transcription, rebuilt on Linux desktop facilities. The original
macOS sources are still in this repository, untouched, under `Sources/`.
...
```bash
git clone https://github.com/Stink-O/better-voice-Linux.git
cd better-voice-Linux
./install.sh
```
